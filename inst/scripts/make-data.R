...
# https://github.com/ELTEbioinformatics/GMT_files_for_mulea/tree/main/scripts_to_create_GMT_files
...
# at
# https://github.com/ELTEbioinformatics/GMT_files_for_mulea/tree/main/scripts_to_create_GMT_files/ID_mapping_scripts
...
download.file(url = "https://github.com/ELTEbioinformatics/GMT_files_for_mulea/archive/master.zip",
              destfile = "../GMT_files_for_mulea-main.zip")

# unzip the .zip file
unzip(zipfile = "../GMT_files_for_mulea-main.zip",
      exdir = "..")

# delete the *.zip file
system("rm ../GMT_files_for_mulea-main.zip")
...
system(paste0("find ", GMT_dir, " -name '.DS_Store' -type f -delete"))
...
# vector with name of the *.zip file(s) and the relative path(s) among the GMT files
zip_files <- system(paste0("find ", GMT_dir, " -name '*.zip'"),
                    intern = TRUE)
...
system(paste0("find ", GMT_dir, " -name '*.zip' -type f -delete"))

# delete the the __MACOSX folders
system(paste0("find ", GMT_dir, " -name '__MACOSX' -type d -exec rm -rf {} +"))
...
process_file <- function(file_path) {
  ...
  destination_file_path <- file.path(destination_path, paste0(file_name, ".rds"))

  # Save the data in RDS format
  saveRDS(gmt_data, file = destination_file_path)

  # Print a message to indicate progress
  cat("File", file_name, "processed and saved to", destination_file_path, "\n")
}
...
#
# locale:
# [1] LC_CTYPE=en_US.UTF-8       LC_NUMERIC=C               LC_TIME=hu_HU.UTF-8        LC_COLLATE=en_US.UTF-8
# [5] LC_MONETARY=hu_HU.UTF-8    LC_MESSAGES=en_US.UTF-8    LC_PAPER=hu_HU.UTF-8       LC_NAME=C
